...
const UploadedEnumSR = () => {
  ...
  const [myUploadedFile] = useState(1); // 마이페이지 내가 업로드에서 넘어갔을때 다운로드버튼으로 활성
  ...
  const handleCLick = (selectedData, myUploadedFile) => {
    navigate("/studentRecordDetail", {state : {item : selectedData, myUploadedFile}})
  };
      ...
      <ContainerBox>
        ...
        <ItemBox>
          ...
          {items.length > 0 ? (items.map((item, index) => (
            ...
            ))) : (<EmptyFileText>업로드한 생활기록부 파일이 없습니다.</EmptyFileText>)
          }
        </ItemBox>
      </ContainerBox>
    </Background>
